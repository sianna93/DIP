...
void main(){
	...
	imgIn=cvLoadImage("img_substraccion.jpg", 1);		//carga la imagen de entrada		
	...
	//Marcar las frecuencias tomadas del histograma a la imagen del histograma de salida
	 ...
	 for(int i=0;i<imgGray->height;i++){
		 for(int j=0;j<imgGray->width;j++){
				 ((uchar*)(imgAlargamiento->imageData))[i*imgAlargamiento->widthStep+j]= (((((uchar*)(imgGray->imageData))[i*imgGray->widthStep+j]-min_freq_hist)/(max_freq_hist-min_freq_hist))*255);
			}
	}

	
	cvShowImage( "Histograma", histograma_salida ); cvWaitKey(0);
	cvShowImage( "Imagen_Alarg", imgAlargamiento ); cvWaitKey(0);

	//Calcula el Histograma de una imagen 8U
   // histograma=cvCreateHist(1,&nBrillo,CV_HIST_ARRAY,NULL,1);
    cvCalcHist(&imgAlargamiento,histograma,0,NULL);
