...
void main(){
	...
	imgIn=cvLoadImage("iguana.jpg", 1);		//carga la imagen de entrada		
	...
	imgCompresion=cvCreateImage(cvGetSize(imgIn),IPL_DEPTH_8U,1);
	...
	//Operaciones para alargamiento
	float shrinkMin = 30,shrinkMax=120;
	...
	for(int i=0;i<imgGray->height;i++){
            for(int j=0;j<imgGray->width;j++){
				((uchar*)(imgCompresion->imageData))[i*imgCompresion->widthStep+j]= (operacion*(((uchar*)(imgGray->imageData))[i*imgGray->widthStep+j]-min_freq_hist)+shrinkMin);
			}
	...
	cvShowImage( "Imagen_Compresion", imgCompresion ); cvWaitKey(0);

	
	//Calcula el Histograma de una imagen 8U
   // histograma=cvCreateHist(1,&nBrillo,CV_HIST_ARRAY,NULL,1);
    cvCalcHist(&imgCompresion,histograma,0,NULL);
